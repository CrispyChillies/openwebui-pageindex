...
class S3StorageProvider(StorageProvider):
    def __init__(self):
        if boto3 is None or Config is None:
            raise RuntimeError(
                "S3 storage provider requires boto3/botocore. Install the package or set STORAGE_PROVIDER=local."
            )
...
class GCSStorageProvider(StorageProvider):
    def __init__(self):
        if storage is None:
            raise RuntimeError(
                "GCS storage provider requires google-cloud-storage. Install the package or set STORAGE_PROVIDER=local."
            )
...
class AzureStorageProvider(StorageProvider):
    def __init__(self):
        if BlobServiceClient is None or DefaultAzureCredential is None:
            raise RuntimeError(
                "Azure storage provider requires azure-storage-blob and azure-identity. Install packages or set STORAGE_PROVIDER=local."
            )
